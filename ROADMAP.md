...
- [x] sha=b32b839 CC28 Add an explicit Docker kernel launcher.
  ...
  > Docker resolution and launch share an immutable image and private session
  > cache. Registry admission records a unique container incarnation before
  > launch; lost replies recover by ownership labels and subsequently pin the
  > immutable container ID. Reconnect never launches, daemon/SSH failure is
  > unknown liveness, and explicit cleanup never signals a Docker host PID.
  > Restart retains connection metadata and the saved image/configuration.
  > Initial support requires the SSH host's Linux Docker daemon, a pre-pulled
  > image, host networking, and the SSH user's UID/GID. Image entrypoint scripts
  > and profile-wide orphan cleanup are unsupported; exact registered-session
  > shutdown/retry-fresh remain available.
  > Strict production byte-compilation and canonical source checks pass:
  > 1170 ERT, 4 array, 260 helper (4 optional skips), 25 registry and 12 fixture
  > tests. The 29 added ERTs include real generated shell commands exercised
  > against an isolated Docker CLI fixture and real registry-worker roundtrips.
  > Generated bytecode removed before the final source run. No actual Docker
  > daemon or remote host smoke test was performed.
...
- [x] sha=4d3b9f6 CC27 Fix panel tracking and preserve the
  ...
  owns resolver validation/tests and launch investigation; root owns profile
  ...
  > Existing output windows are reused; malformed display rules fall back
  > safely for output and explicit source navigation. Hidden-window tracking
  > remains passive. Failed start, prefixed evaluation and retry-fresh retain
  > the buffer's selected profile; retired synchronous registry callbacks
  > cannot overwrite the next acquisition context. Direct Docker resolver
  > commands fail clearly before returning container paths for host launch.
  > Strict production byte-compilation and canonical source checks pass:
  > 1141 ERT, 4 array, 260 helper (4 optional skips), 25 registry and 12
  > fixture tests. Generated bytecode removed before the final source run.
  > No remote host or Docker smoke test was needed or performed.
...
- [x] sha=6cd9d42 CC26 Recover evaluation from disconnected
  ...
  > Unsent FIFO records survive local transport replacement. Stale local
  > handles and remote liveness evidence trigger asynchronous verification;
  > confirmed-dead replacement preserves the initiating evaluation through
  > consent and exact registry retirement. Fifteen new ERT regressions cover
  > callback joining, cancellation, PID reuse, registry conflicts and deadlines.
  > Final source checks pass: 1120 ERT, 4 array, 260 helper (4 optional skips),
  > 25 registry and 12 fixture tests. Strict production compilation passes;
  > generated bytecode was removed before the final canonical source rerun.
  > No real remote outage smoke was required or performed.

- [x] sha=6cd9d42 CC25 Draw linked line ROIs and compare
  ...
  > Directed A/B lines synchronize across corresponding panes and plot the
  > selected line's original-sample profiles, including pinned/difference
  > images. Bilinear sampling and plot reservations are bounded; old results
  > are discarded on movement, selection, replacement and close. Viewer suite
  > passes 111 tests in both source and pinned Nix checks; all 12 line GUI tests
  > also pass with native Linux Qt under Xvfb. The packaged paint/zoom/resize/
  > reopen smoke passes. Independent review found no remaining blocker.
...
- [x] sha=b82fc55 CC23 Give the output buffer ordinary
  ...
  panel_review owns window tests and reviews the implementation. Preserve
  output rendering and kernel lifetime.
  > Verified seven real-window regressions and independent implementation
  > review; strict Elisp compilation and canonical source-based local checks
  > passed: 1105 ERT, 4 array protocol, 260 helper (4 optional skips), 25
  > registry and 12 fixture tests. Generated bytecode removed before ERT.
  > Existing side windows need closing/reopening after reloading EJN.
...
- [x] sha=4adc3a9 CC22 Investigate reported SSH mux broken
  ...
  > Verified target tunnel isolation with 20 local `ssh -G` cases, including
  > inherited options and explicit `-o ControlPath` / `-S`. Independent review
  > passed. ProxyJump's master retains the jump host configuration; the user's
  > simultaneous terminal drops on another machine remain unattributed.

- [x] sha=4adc3a9 CC21 Preserve running execution output
  ...
  > Verification: 1098/1098 source ERT, 260 helper (4 dependency skips),
  > 4 array protocol, 25 registry and 12 fixture tests passed; strict Elisp
  > compilation and helper Nix package build passed with no generated bytecode
  > retained. Four real local-kernel relay tests cover resumed output, offline
  > completion, safe local close and renewed stdin leases. Independent review
  > found and regressed stale acknowledgement, grace-timer and cleanup races;
  > fixes passed all gates. Missing offline output is explicitly disclosed.
...
- [x] sha=4803b32 CC20 Make notebook mode setup idempotent
  ...
  This is separate from the reported plain-Python incident, where EJN was not
  enabled. Assigned
  ...
  > Verification: seven new lifecycle regressions, including actual outline
  > fontification, fail before the fix and pass afterwards; independent review
  > found no blocking issue. Strict compilation and canonical source checks
  > passed: 1074 ERT, 4 array protocol, 248 helper (4 optional skips), 25 registry
  > and 12 fixture tests. No generated bytecode retained.
...
- [x] sha=2ef2e80 CC19 Review and eliminate probable Emacs
  ...
  > Verification: 1067/1067 source ERT, 4 array-protocol, 248 helper (4 optional
  > skips), 25 registry and 12 fixture tests passed; strict byte compilation
  > and four local AG3 subprocess stress tests passed. No generated bytecode.
  > The reported machine's preconnection minibuffer input failure remains
  > unattributed; it also occurs with Vertico disabled. Full review and limits:
  > `docs/hang-review-2026-09-09.md`.
...
- [x] sha=4b2934e CC16 Make variable metadata a live
...
- [x] sha=4b2934e CC17 Add pinned-reference/follow/freeze/
...
- [x] sha=4b2934e CC18 Integrate the seven requested UX
  ...
  > Verification: 1032 ERT, 248 helper, 4 array-protocol, 25 registry and 12
  > stress tests passed; viewer 95 passed/1 optional skip; two real local-kernel
  > tests, actual Linux compact GUI, Evil bindings, pinned Nix builds/checks
  > and offline Darwin packaging passed. See
  > `docs/dogfood/2026-09-09-notebook-ux.md`. Independent review findings fixed;
  > full W21 macOS/native-pixel and remote-slider gates remain separate.
...
- [x] sha=ee4a974 CC11 Stable linked image navigation and
  ...
  center/scale together without intermediate aspect-ratio rescaling. Test
  intermediate navigation, geometry changes and incompatible grids. Keep all
  navigation local and preserve numerical samples; real macOS gesture sign-off
  remains a user-machine check.
...
- [x] sha=8529816 CC3 Make external image snapshot bounds
...
- [x] owner=root claimed=2026-09-03 sha=00d66db CC6 Prevent large image output from
      ...
      `helper/ejn_helper/jupyter_backend.py`, `emacs-jupyter-notebook.el`,
      focused helper/formatter tests — for: W8/W15/EI7
...
- [x] sha=6a6fd4c W2.7 Rewrite `emacs-jupyter-notebook-jupyter--callbacks` to drive the
...
- [x] sha=3e96521 W2.8 Fringe/margin indicator. Per cell-marker overlay reflecting the
...
- [x] sha=4d62f65 W4.2 Replace the unanchored `[0-9]+` PID parse with a printed sentinel.
...
- [x] sha=4f0d85e W5.2 `--evaluation-timer` enforces: on timeout, call
      ...
      interrupt adapter.
- [x] sha=cae057d W5.3 `cancel-operation` learns about in-flight evaluations: if
...
- [x] sha=d50dd38 W6.2 Mode-line lighter is a function returning one of:
...
- [x] sha=c0c615d W6.5 Status command renders to a `special-mode` buffer
...
- [x] sha=34f4395 W7.4 Doom e2e additions: after the existing image assertion, run a
...
- [x] sha=fc44153 W8.1 Emacs injects the one-time formatter-registration snippet into
...
- [x] sha=f9735d7 W8.2 Emacs MIME recognition: teach the `display_data` /
      ...
      unchanged.
- [x] sha=c98a50f W8.3 Local viewer process manager (`emacs-jupyter-notebook-viewer.el`):
...
- [x] sha=f17a069 W8.4 The viewer Python script (`viewer/ejn_viewer.py`): persistent
      ...
      manual).
- [x] sha=9181ea6 W8.5 Command + binding: `C-c j I` and `v` on a panel plot entry open
...
- [x] sha=8265405 W10 Connection-lifecycle field-bug fix — a source buffer was
...
- [x] sha=bb135d0 W9 object-attribute completion (`df.` / `obj.`)
      ...
      Post-merge (opencode W9 review): tightened
      `--completion-explicit-command-p` to key on the `complet' verb (plus a
      `company-manual-begin' memq) instead of bare `corfu'/`company' prefixes,
