...
(ert-deftest ejn-callback-clear-output-immediate ()
  (with-temp-buffer
    (let* ((buffer (current-buffer))
           ...
           (callbacks (emacs-jupyter-notebook-jupyter--callbacks buffer ov (point-min)))
...
(ert-deftest ejn-status-message-sets-kernel-status ()
  (with-temp-buffer
    ...
    (let ((buffer (current-buffer))
      ...
      (let* ((callbacks (emacs-jupyter-notebook-jupyter--callbacks buffer ov (point-min)))
...
(ert-deftest ejn-execution-count-overlay-created-with-correct-text ()
  (with-temp-buffer
    (insert "# %%\nx = 1\n")
    (let ((pos (point-min)))
      (emacs-jupyter-notebook-result--set-execution-count pos 42)
      (should (= (length emacs-jupyter-notebook-result--execution-count-overlays) 1))
      (let ((ov (car emacs-jupyter-notebook-result--execution-count-overlays)))
        (should (overlayp ov))
        (should (overlay-get ov 'emacs-jupyter-notebook-execution-count))
        (should (= (overlay-get ov 'emacs-jupyter-notebook-execution-count-pos) pos))
        (should (string-match-p "\\[42\\]" (overlay-get ov 'before-string)))))))

(ert-deftest ejn-busy-indicator-shows-star ()
  (with-temp-buffer
    (insert "# %%\nx = 1\n")
    (let ((pos (point-min)))
      (emacs-jupyter-notebook-result--set-busy-indicator pos)
      (should (= (length emacs-jupyter-notebook-result--execution-count-overlays) 1))
      (let ((ov (car emacs-jupyter-notebook-result--execution-count-overlays)))
...
(ert-deftest ejn-clear-results-removes-execution-count-overlays ()
  (with-temp-buffer
    (insert "# %%\nx = 1\n# %%\ny = 2\n")
    (emacs-jupyter-notebook-result--set-execution-count (point-min) 1)
    (emacs-jupyter-notebook-result--set-execution-count (point-min) 2)
    (should (= (length emacs-jupyter-notebook-result--execution-count-overlays) 1))
    (emacs-jupyter-notebook-result-clear-all)
    (should (= (length emacs-jupyter-notebook-result--execution-count-overlays) 0))))
...
(ert-deftest ejn-execution-count-replaced-on-reevaluation ()
  (with-temp-buffer
    ...
    (let ((pos (point-min)))
      (emacs-jupyter-notebook-result--set-busy-indicator pos)
      (should (string-match-p "\\[\\*\\]"
                              (overlay-get (car emacs-jupyter-notebook-result--execution-count-overlays)
                                           'before-string)))
      (emacs-jupyter-notebook-result--set-execution-count pos 5)
      (should (= (length emacs-jupyter-notebook-result--execution-count-overlays) 1))
...
(ert-deftest ejn-callback-execute-reply-sets-execution-count ()
  (with-temp-buffer
    ...
    (let* ((buffer (current-buffer))
           (cell-beg (point-min))
           (ov (emacs-jupyter-notebook-result-start (point-min) (point-max)))
           (callbacks (emacs-jupyter-notebook-jupyter--callbacks buffer ov cell-beg))
           (reply-fn (cadr (assoc "execute_reply" callbacks))))
      (emacs-jupyter-notebook-result--set-busy-indicator cell-beg)
      (should (string-match-p "\\[\\*\\]"
                              (overlay-get (car emacs-jupyter-notebook-result--execution-count-overlays)
                                           'before-string)))
      (cl-letf (((symbol-function 'jupyter-message-content)
...
(ert-deftest ejn-clear-region-removes-execution-count-overlays ()
  (with-temp-buffer
    ...
    (let ((pos1 (point-min))
      ...
      (emacs-jupyter-notebook-result--set-execution-count pos1 1)
      (emacs-jupyter-notebook-result--set-execution-count pos2 2)
      (should (= (length emacs-jupyter-notebook-result--execution-count-overlays) 2))
  ...
  (with-temp-buffer
    (let* ((buffer (current-buffer))
           ...
           (callbacks (emacs-jupyter-notebook-jupyter--callbacks
                       buffer ov (point-min) 'mock-client))
